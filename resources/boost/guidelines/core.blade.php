...
The `misaf/vendra-reseller` package owns the **reseller domain and the reseller self-service panel**. A reseller is the billing owner of one or more properties, so this package sits above `misaf/vendra-property` and below `misaf/vendra-console`.
...
- `Models\Reseller` is the platform's `SubscriptionSubscriber`. Plan limits and property quotas are answered by `misaf/vendra-subscription` and `Misaf\VendraProperty\Support\PropertyQuota`; do not reimplement limit arithmetic in a page, widget, or action.
...
- Subscription reactions are wired in `Providers\ResellerServiceProvider`: the subscription engine raises only generic lifecycle events, and this package turns them into reseller behaviour (`NotifyActivatedSubscriber`, `RemindExpiringSubscriber`, `SuspendSubscriberProperties`). Add a new reaction as a listener here, not as domain logic inside the subscription engine, and do not also register these listeners in the host app.
- Property work is delegated, not duplicated: the panel's resources extend `misaf/vendra-property`'s `CreatePropertyPage`, `StorefrontConfigurationFields`, and `ReplaceDomainAction`. The only reseller-specific part is which reseller is resolved as the owner. `Console\Commands\ProvisionPropertyCommand` calls `ProvisionPropertyAction` for the same reason.
